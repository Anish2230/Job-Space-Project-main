...
export const login = async (req, res) => {
    try {
        ...
        const token = await jwt.sign(tokenData, jwt_key , { expiresIn: '1d' });
      ...
      .cookie("token", token, {
        ...
        secure: true, 
        sameSite: "None", // Adjust for dev/prod
...
/*
        res.setHeader("Set-Cookie", `token=${token}; Max-Age=${24 * 60 * 60}; HttpOnly; Path=/; SameSite=Strict${process.env.NODE_ENV === "production" ? "; Secure" : ""}`);
        return res.status(200).json({
            message: `Welcome back ${user.fullname}`,
            user,
            success: true,
        ...
        return res.status(200).cookie("token", token, { maxAge: 1 * 24 * 60 * 60 * 1000, httpOnly: true, secure: process.env.NODE_ENV === "production" , path: "/" , sameSite: 'strict' }).json({
            message: `Welcome back ${user.fullname}`,
            user,
            success: true
        })
